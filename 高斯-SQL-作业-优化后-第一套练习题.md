...
mydb=# 
select 	
	sid,sname,to_char(sbirthday),ssex 
		from 
			student 
				where 
					to_char 
...
select 
	sid,sum(score) 
		...
		where 
			sid in 
				(select sid from student where sname='Megan') 
		group by sid;
 sid | sum  
-----+------
 06  | 65.0
...
from 
	student t3
		right join 
	(select t.sid,t.s as s1 from (select sid,sum(score) as s  from score  group by sid order by s desc) t where s1 > 190) t4
...
mydb=# 
select t1.sname,t2.score from (select sid,cid,score from (select sid,cid,score from score  where cid in (select cid from course where cname = 'Math')) t where t.score > 87) t2 left join student t1 on t2.sid = t1.sid;
 sname | score 
-------+-------
 Bobby |  90.0
 Gina  |  89.0
...
老师给的参考：
select 
	s1.sname,s2.score as ts
from 
	student s1,score s2,course c1 
where 
	s1.sid = s2.sid 
	and s2.cid = c1.cid 
and 
